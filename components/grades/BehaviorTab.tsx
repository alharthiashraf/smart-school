...
function latestEvents(
  ...
  return events
    .filter(
      (event) =>
        event.student_id === studentId && event.score_type === "behavior"
    )
    .slice(0, limit);
}
    ...
    <section className="overflow-hidden rounded-[1.4rem] border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <div className="mt-1 text-xs font-bold text-slate-200">
              مستقل تمامًا عن المواد — الدرجة الأساسية 100 — التعديل بالخصم أو الإضافة.
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Stat icon={Users} label="الطلاب" value={rows.length} />
            <Stat icon={UserCheck} label="ممتاز" value={excellent} />
      ...
      {critical > 0 && (
        <div className="border-b border-red-100 bg-red-50 px-4 py-3 text-xs font-black text-red-700">
          يوجد {critical} طالب بدرجة سلوك حرجة أقل من 60، يفضّل مراجعة السجل واتخاذ إجراء تربوي.
        </div>
      )}
        ...
        <table className="w-full min-w-[980px] table-fixed border-collapse text-xs">
          ...
          <thead className="sticky top-0 z-20">
            <tr className="bg-slate-100 text-slate-700">
              ...
              <th className="p-2 text-center font-black">درجة السلوك</th>
                    ...
                    <td className="p-2 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            onOpenAction(row.student, "behavior", "decrease")
                          }
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            onOpenAction(row.student, "behavior", "increase")
                          }
